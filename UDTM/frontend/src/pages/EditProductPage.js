...
const EditProductPage = ({ userInfo: userInfoProp }) => {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem("userInfo")) || userInfoProp;
    if (!stored || (stored.role !== "seller" && stored.role !== "admin")) {
      alert("Chỉ seller hoặc admin mới có quyền chỉnh sửa sản phẩm");
      navigate("/home");
      return;
    }
    setUserInfo(stored);
    ...
    api.get(`/products/${productId}`)
      .then((res) => {
        ...
        if (product.photo) setImagePreview(product.photo);
        if (product.trailer) setTrailerUrl(product.trailer);
      })
      .catch((err) => {
        alert("Lỗi tải sản phẩm: " + err.message);
        if (stored && stored.role === "admin") navigate("/admin/products");
        else navigate("/seller");
      })
      .finally(() => setLoading(false));
  }, [navigate, productId, userInfoProp]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (!uploadedImage) return setSubmitting(false);
        photoUrl = uploadedImage;
      }

      if (trailerFile) {
        const uploadedTrailer = await uploadTrailerFile();
        if (!uploadedTrailer) return setSubmitting(false);
        finalTrailerUrl = uploadedTrailer;
      }

      let res;
      if (userInfo.role === "admin") {
        res = await api.put(`/products/${productId}`, {
        ...
        res = await api.patch(`/seller/products/${productId}`, {
          product_name: formData.product_name,
          short_description: formData.short_description,
          description: formData.description,
          price: parseFloat(formData.price),
      ...
      if (res && res.status >= 200 && res.status < 300) {
        ...
        if (userInfo.role === "admin") navigate("/admin/products");
        else navigate("/seller");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#c5e7fb] via-[#a2c4f8] to-[#ffdbe1] py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <button
            onClick={() => (userInfo.role === "admin" ? navigate("/admin/products") : navigate("/seller"))}
            className="mb-4 flex items-center gap-2 font-semibold"
            style={{ color: "#313b7d" }}
          >
            ← Quay lại
          </button>
          <h1 className="text-3xl font-bold" style={{ color: "#313b7d" }}>
            Chỉnh sửa sản phẩm
          </h1>
          <p className="text-sm text-gray-600 mt-1">
            {userInfo.role === "admin" ? "Admin - Chỉnh sửa bất kỳ sản phẩm nào" : "Seller - Chỉnh sửa sản phẩm của bạn"}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4">
              ...
              {imagePreview && (
                <img src={imagePreview} alt="Preview" className="w-24 h-24 object-cover rounded-lg border border-gray-300" />
              )}
            </div>
          </div>
                ...
                <div className="mt-3 bg-gray-100 p-3 rounded-lg">
                  {trailerPreview ? (
                    ...
                    <iframe
                      width="100%"
                      height="200"
                      src={`https://www.youtube.com/embed/${trailerUrl.split("v=")[1]?.split("&")[0]}`}
                      title="Video Preview"
                      allowFullScreen
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              disabled={submitting}
              style={{ backgroundColor: "#313b7d" }}
              className="flex-1 py-2 rounded-lg font-semibold text-white hover:opacity-90 transition disabled:opacity-50"
            >
            ...
            <button
              type="button"
              onClick={() => (userInfo.role === "admin" ? navigate("/admin/products") : navigate("/seller"))}
              className="flex-1 py-2 rounded-lg bg-slate-300 text-gray-700 hover:bg-slate-400 transition"
            >
              Hủy
            </button>
          </div>
